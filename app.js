...
// app.post('/rooms/:id/images',
//   uploadRoomImages.array('images', 10), async (req, res) => {
//   // req.files-ը կպարունակի Cloudinary-ի URL-ները
//     console.log(req)
//   res.json({ files: req.files });
// });


export function parseAll(apiData) {
  const result = {};

  // 🔹 1. HOTEL NAME
  result.name =
    apiData?.propertyContentSectionGroups?.aboutThisProperty?.sections?.[0]
      ?.bodySubSections?.[0]?.elements?.[0]?.header?.text || "";
  ...
  const amenitySections =
    apiData?.propertyContentSectionGroups?.amenities?.sections || [];

  for (const section of amenitySections) {
    const category = section?.header?.text || "General";
  ...
  // 🔹 4. EXTRAS + POLICIES (HTML parse)
  const policiesHTML =
    apiData?.propertyContentSectionGroups?.policies?.sections?.[0]
      ?.bodySubSections?.[0]?.elements?.[0]?.items?.[0]?.content?.text || "";

  result.extras = [];
  result.policies = policiesHTML;

  if (policiesHTML) {
    const $ = cheerio.load(policiesHTML);
  ...
  // 🔹 5. LOCATION (եթե ունես nearby data ուրիշ API-ից)
  result.locations = []; // later fill

  // 🔹 6. REVIEWS (եթե կա ուրիշ API-ում)
  result.reviews = [];

  // 🔹 7. ROOM (hardcoded կամ ուրիշ response-ից)
  result.room = {
  ...
  //     description: 'Take in the views from the rooftop terrace when you stay at Paris Hotel Ye
  //   revan. Guests can indulge in mani/pedis, and Montmartre Restaurant serves Fusion cuisine.
  //   Other highlights include a bar/lounge, a fitness center, and a terrace. Fellow travelers say great things about the helpful staff.   ',
  //   amenities: [],
  //     extras: [
  //   {
  //     name: 'Wired internet access is available in guestrooms for an extra charge',
  //     price: 0
  //   },
  //   {
  //     name: 'Airport shuttle service is offered for an extra charge of AMD 8000 per vehicle (one way)',
  //     price: 8000
  //   },
  //   {
  //     name: 'Early check-in can be arranged for an extra charge (subject to availability)',
  //     price: 0
  //   }
  // ],
  //   policies: '<ul><li><strong>Wired internet</strong> access is available in guestrooms for
  //   an extra charge</li><li><strong>Airport shuttle</strong> service is offered for an extra
  //   charge of AMD 8000 per vehicle (one way)</li><li><strong>Early check-in</strong> can be arranged for an extra charge (subject to availability)</li></ul>',
  //   locations: [],
  //     reviews: [],
  //   room: {
  //   name: 'Standard Room, 1 King Bed',
...
app.get('/api/hello', (req, res) => {
  res.json({message: 'Բարև frontend!'});
});
...
//           transportTypes: transportTypes || 'FLIGHT',
//           contentProviders: contentProviders || 'FLIXBUS_DIRECTS,FRESH,KAYAK,KIWI',
//           price_from: priceStart || undefined,
//           price_to: priceEnd || undefined,
//           max_stopovers: maxStops || undefined,
//           outbound_departure_from: outboundDepartureDateStart || undefined,
//           outbound_departure_to: outboundDepartureDateEnd || undefined,
//           inbound_departure_from: inboundDepartureDateStart || undefined,
//           inbound_departure_to: inboundDepartureDateEnd || undefined,
...
//     });
//
//     res.json(formatItinerariesResponse(response.data, page, limit));
//   } catch (err) {
//     res.status(500).json({error: err.response?.data || err.message});
//   }
// });
...
// petq amboxjn
app.get('/hotels/api', async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Query Overpass API for hotels
...
`;
    ...
    const hotels = overpassRes.data.elements.map(hotel => hotel);
  ...
  return data.categorizedListings.map(unit => {
    ...
    return {
      ...
      extras: secondary
        ...
        .filter(e => e.id), // 👈 սա հանում է null / undefined
...
app.get('/hotel-offers', async (req, res) => {
  ...
  try {
    const response = await axios.get(
      'https://hotels-com-provider.p.rapidapi.com/v2/hotels/offers',
      {
        params: {
          hotel_id,
    ...
    //
    // if (offersData.soldOut || !offersData.listings?.length) {
    //   return res.json({
    //     soldOut: true,
    //     message: offersData.errorMessage?.text || "No rooms available"
    //   });
    // }


// Otherwise send available offers
//     res.json({soldOut: false, offers: offersData.listings});
    const rooms =
      response?.data?.categorizedListings?.map(r => ({
        id: r?.unitId,
        name: r?.header?.text,
        ...
        ratePlans: r?.ratePlans?.map(p => ({
          id: p?.id,
          badge: p?.badge?.text,
          discount: p?.badge?.text,
          price: p?.priceDetails?.[0]?.price,
          paymentPolicy: p?.paymentPolicy
        })) || [],
        images: r?.primarySelections?.[0]?.propertyUnit?.unitGallery?.gallery
          ?.map(i => i?.image?.url)
          .filter(Boolean)
      })) || [];

    secondarySelections: response?.data?.secondarySelections?.map(sec => ({
      policy: sec.secondarySelection?.description,
      policyPrice: sec.secondarySelection?.price,

      o: response?.data?.secondarySelections,

      extras: response?.data?.tertiarySelections?.map(t => ({
        id: t.optionId,
        name: t.description,
        price: t.price,
        selected: t.selected,
    ...
    // res.json(rooms);
    res.json(mapHotelData(response.data));
...
app.get('/hotel-info', async (req, res) => {
  ...
  try {
    const response = await axios.get(
      'https://hotels-com-provider.p.rapidapi.com/v2/hotels/info',
      {
        params: {
          hotel_id,
          locale: 'en_US',
    ...
    const hotel = {
      ...
      flatFacts: data?.propertyContentSectionGroups?.amenities?.sections?.[0]?.bodySubSections
        ?.flatMap(section =>
          section?.elements?.flatMap(el =>
            el?.items?.map(i => ({
              title: el?.header?.text,
              value: i?.content?.primary?.value
            }))
          )
        )
        ?.filter(x => x?.value) || [],
      ...
      policies: data?.propertyContentSectionGroups?.policies?.sections?.[0]?.bodySubSections
        ?.flatMap(s =>
      ...
      specialFeatures: data.propertyContentSectionGroups?.specialFeatures?.sections[0]?.bodySubSections[0]?.elements[0]?.items[0]?.content?.text
    };
...
function cleanTopAmenities(data) {
  return (data?.summary?.amenities?.topAmenities?.items || [])
    .map(i => ({
      name: i?.text,
      icon: i?.icon?.token || null
    }))
    .filter(i => i.name);
}


export function cleanPolicies(data) {
  const p = data?.summary?.policies;

  return {
    checkin: {
      instructions: p?.checkinInstructions || [],
      minAge: p?.checkinMinAge || null,
      start: p?.checkinStart || null,
      end: p?.checkinEnd || null
    },

    child: (p?.childAndBed?.body || []).filter(Boolean),

    pets: (p?.pets?.body || []).filter(Boolean),

    needToKnow: (p?.needToKnow?.body || []).filter(Boolean),
...
app.get('/hotel-details', async (req, res) => {
  ...
  try {
    const response = await axios.get(
      ...
      {
        params: {hotel_id, domain: 'US', locale: 'en_US'},
        headers: {
          'Content-Type': 'application/json',
          'x-rapidapi-host': 'hotels-com-provider.p.rapidapi.com',
          'x-rapidapi-key': "08daf8401amsha133c8f117f5883p1f2bafjsnd4c886293eca"
        }
      }
    );
    const data = response.data;
    ...
    if (response?.data && Object.keys(response.data).length) {
      hotel = {
        ...
        hotelPolicies:
          (data?.propertyContentSectionGroups?.policies?.sections?.[0]?.bodySubSections
            ?.flatMap(section =>
              section?.elements?.flatMap(el =>
                el?.items?.map(item => ({
                  title: el?.header?.text || '',
                  text: item?.content?.text?.replace(/<[^>]*>/g, '').trim() || ''
                }))
              )
            )
            .filter(x => x?.text)) || [],
        ...
        hotelImages: data?.propertyGallery?.images?.map(img => ({
          accessibilityText: img.accessibilityText,
          id: img.imageId,
          url: img.image?.url,
          description: img.image?.description || img.accessibilityText || ''
        })) || [],

        reviews: {
          guestRating: data?.reviewInfo?.summary?.overallScoreWithDescriptionA11y?.value || '',
          reviewCount: data?.reviewInfo?.summary?.propertyReviewCountDetails?.shortDescription || ''
        },


        amenities:
          data?.summary?.amenities?.amenities?.flatMap(section =>
            section?.contents?.flatMap(c =>
        ...
        location: {
          addressLine: data?.summary?.location?.address?.addressLine,
          city: data?.summary?.location?.address?.city,
          countryCode: data?.summary?.location?.address?.countryCode,
          firstAddressLine: data?.summary?.location?.address?.firstAddressLine,
          secondAddressLine: data?.summary?.location?.address?.secondAddressLine
        },
        ...
        whatsAround: data?.summary?.whatsAround?.editorial?.content,

        name: data?.summary?.name,

        nearbyPOIs:
          data?.summary?.nearbyPOIs?.items?.map(i => ({
            text: i?.text,
            moreInfo: i?.moreInfo,
          })) || [],

        propertyRating: {
          accessibility: data?.summary?.overview?.propertyRating?.accessibility || null,
          rating: data?.summary?.overview?.propertyRating?.rating || null
        },

        policies: {
          checkin: data?.summary?.policies?.checkinInstructions || [],
          childAndBed: data?.summary?.policies?.childAndBed?.body || [],
          pets: data?.summary?.policies?.pets?.body || [],
          needToKnow: data?.summary?.policies?.needToKnow?.body || [],
          shouldMention: data?.summary?.policies?.shouldMention?.body || [],
        }
      };
    }


    // Transform nested API data for frontend
    // const hotel = {
    //
    //   // a: data.propertyContentSectionGroups,
    //
    //
    //   aboutThisProperty:{
    //     co: data?.propertyContentSectionGroups?.aboutThisProperty?.sections[0]?.bodySubSections[0]
    //     // content: data.propertyContentSectionGroups.aboutThisProperty.sections[0].bodySubSections[0].elements[0].items[0].content.text,
    //     // text: data.propertyContentSectionGroups.aboutThisProperty.sections[0].bodySubSections[0].elements[0].header.text
    //   },
    //
    //   hotelPolicies: data?.propertyContentSectionGroups?.policies?.sections[0]?.bodySubSections,
    //
    //   hotelImages: data?.propertyContentSectionGroups?.propertyGallery?.images?.map(img => ({
    //
    //     accessibilityText: img.accessibilityText,
    //     id: img.imageId,
    //     url: img.image?.url,
    ...
    //
    //   reviews: {
    //     guestRating: data?.propertyContentSectionGroups?.reviewInfo?.summary?.overallScoreWithDescriptionA11y?.value || '',
    //     reviewCount: data?.propertyContentSectionGroups?.reviewInfo?.summary?.propertyReviewCountDetails?.shortDescription || ''
    //   },
    //
    //     amenities:data?.propertyContentSectionGroups?.summary?.amenities?.amenities,
    //     // topAmenities:cleanTopAmenities(data.propertyContentSectionGroups.summary.amenities.items),
    //
    //   location:{
    //     addressLine: data?.propertyContentSectionGroups?.summary?.location?.address?.addressLine,
    //     city: data?.propertyContentSectionGroups?.summary?.location?.address?.city,
    //     countryCode: data?.propertyContentSectionGroups?.summary?.location?.address?.countryCode,
    //     firstAddressLine:data?.propertyContentSectionGroups?.summary?.location?.address?.firstAddressLine,
    //     secondAddressLine:data?.propertyContentSectionGroups?.summary?.location?.address?.secondAddressLine
    //   },
    //
    //
    //   coordinates: {
    //     latitude: data?.propertyContentSectionGroups?.summary?.coordinates?.latitude,
    //     longitude:data?.propertyContentSectionGroups?.summary?.coordinates?.longitude
    //   },
    //
    //   whatsAround: data?.propertyContentSectionGroups?.summary?.whatsAround?.editorial?.content,
    //
    //   name: data?.propertyContentSectionGroups?.summary?.name,
    //
    //   nearbyPOIs: data?.propertyContentSectionGroups?.summary?.nearbyPOIs?.items,
    //
    //   propertyRating:{
    //     accessibility: data?.propertyContentSectionGroups?.summary?.overview?.propertyRating?.accessibility,
    //     rating: data?.propertyContentSectionGroups?.summary?.overview?.propertyRating?.rating
    //
    //   },
    //
    //
    //   policies:
    //     // checkinInstructions: data.propertyContentSectionGroups.summary.policies.checkinInstructions,
    //     cleanPolicies(data?.propertyContentSectionGroups?.summary?.policies)
    //
    //
    //
    //   // id: hotel_id,
    //   // name: data.name || data.overview?.label || data.propertyContentSectionGroups?.aboutThisProperty?.header?.text || '',
    //   // address: data.location?.address?.firstAddressLine || '',
    //   // city: data.location?.address?.city || '',
    //   // country: data.location?.address?.countryCode || '',
    //   // starRating: data.starRating || data.starRatingIcon?.token || '',
    //   // guestRating: data.reviewInfo?.summary?.overallScoreWithDescriptionA11y?.value || '',
    //   // reviewCount: data.reviewInfo?.propertyReviewCountDetails?.totalReviews || 0,
    //   //
    //   // amenities: (data.propertyContentSectionGroups?.amenities?.sections?.flatMap(
    //   //   s => s.contents?.flatMap(c => c.items?.map(i => i.text)).filter(Boolean) || []
    //   // ) || []).filter(Boolean),
    //   //
    //   // policies: (data.propertyContentSectionGroups?.policies?.sections?.map(section => {
    //   //   const title = section.sectionName || section.header?.text || '';
    //   //   const details = (section.bodySubSections || []).map(sub => sub.text).filter(Boolean);
    //   //   return { title, details };
    //   // }) || []).filter(Boolean),
    //   //
    //   // specialFeatures: (data.propertyContentSectionGroups?.specialFeatures?.sections?.flatMap(
    //   //   s => s.contents?.flatMap(c => c.items?.map(i => i.text)).filter(Boolean) || []
    //   // ) || []).filter(Boolean),
    //   //
    //
...
app.get("/hotel-summary", async (req, res) => {
  const {hotel_id} = req.query;
  ...
  try {
    ...
    const response = await axios.get(
      "https://hotels-com-provider.p.rapidapi.com/v2/hotels/summary",
      {
        params: {
          hotel_id,
    ...
    //   id: hotel_id,
    //   name: data.name,
    //   categories: grouped, // 👈 ամենակարևոր մասը
    // });
  } catch (err) {
    console.error(err.response?.data || err.message);
...
//
// const url = 'https://hotels-com-provider.p.rapidapi.com/v2/meta/convert/slug-id';
// // const params = { slug: 'ho606670' }; // Ուղարկվող query parameter
// const params = { slug: 'ho3276717472' }; // Ուղարկվող query parameter
//
// const headers = {
//   'Content-Type': 'application/json',
//   'x-rapidapi-host': 'hotels-com-provider.p.rapidapi.com',
//   'x-rapidapi-key': process.env.RAPIDAPI_KEY // Փոխարինեք ձեր իրական RapidAPI Key-ով
// };
//
// axios.get(url, { headers, params })
//   .then(response => {
//     console.log('Պատասխան՝', response.data);
//   })
//   .catch(error => {
//     if (error.response) {
//       // Սերվերի կողմից վերադարձված սխալ
//       console.error('Սերվերի սխալ՝', error.response.status, error.response.data);
//     } else {
//       // Ցանցի կամ այլ սխալ
...
// app.get('/hotels', async (req, res) => {
//   const { checkin, checkout, adults, children, stars, priceMin, priceMax } = req.query;
//
//   try {
//     const url = 'https://hotels-com-provider.p.rapidapi.com/v2/hotels/search';
//
//     const params = {
//       page_number: '1',
//       checkin_date: checkin || '2026-05-26',
//       checkout_date: checkout || '2026-05-27',
//       adults_number: adults || '1',
//       children_ages: children || '',
//       star_rating_ids: stars || '3,4,5',
//       price_min: priceMin || '10',
//       price_max: priceMax || '500',
//       sort_order: 'REVIEW',
//       available_filter: 'SHOW_AVAILABLE_ONLY',
...
// ✅ Express route, որը React-ից կանչելու համար է
...
//     const $ = cheerio.load(html);
//
//     // 1️⃣ Վերլուծում ենք warmup data JSON
//     const warmupScript = $("#wix-warmup-data").html();
//     if (!warmupScript) {
//       console.log("Warmup data չի գտնվել");
//       return [];
//     }
//
//     const warmupData = JSON.parse(warmupScript);
...
//     const photos = new Set();
//
//     // 2️⃣ Ընտրում ենք բոլոր WPhoto կոմպոնենտները
//     for (const compId in comps) {
//       if (comps[compId] === "WPhoto") {
//         $(`#${compId} img`).each((i, el) => {
//           const src = $(el).attr("src");
...
//     console.log("Նկարների քանակը:", photosArray.length);
//     console.log(photosArray);
//
//     return photosArray;
//
//   } catch (err) {
//     console.error("Սխալ նկարի ստացման ժամանակ:", err.message);
//     return [];
//   }
// }
//
// // Օրինակ օգտագործում
// getWixPhotos("https://www.vmodhotelyerevan.com/room-photos");


// app.get("/hotels", async (req, res) => {
//   const { regionId, checkin, checkout, adults } = req.query;
//
//   // Ստուգում պարտադիր param-երը
//   if (!regionId) return res.status(400).json({ error: "regionId is required" });
//
//   // Եթե չկան date-երը, դնում ենք default՝ վաղը մեկ գիշեր
//   const checkinDate = checkin || new Date(Date.now() + 24*60*60*1000).toISOString().split("T")[0];
//   const checkoutDate = checkout || new Date(Date.now() + 2*24*60*60*1000).toISOString().split("T")[0];
//   const adultsNumber = adults || 1;
//
//   try {
//     const response = await axios.get(
//       "https://hotels-com-provider.p.rapidapi.com/v2/hotels/list",
//       {
//         params: {
//           destination_id: regionId,
//           locale: "es_AR",
//           domain: "AR",
...
//     // Վերադարձնում ենք միայն կարևոր դաշտերը
//     const hotels = response.data.data.body.searchResults.results.map(h => ({h
//       // id: h.id,
//       // name: h.name,
//       // address: h.address?.streetAddress || "N/A",
//       // city: h.address?.city || "N/A",
//       // starRating: h.starRating || "N/A",
//     }));
//
//     res.json(hotels);
//   } catch (err) {
...
//     );
//
//     const hotels = response.data.data.body.searchResults.results.map(h => ({
//       id: h.id,
//       name: h.name,
...
//     //   id: hotel_id,
//     //   name: data.name || 'N/A',
//     //   address: data.location?.address?.streetAddress || 'N/A',
//     //   city: data.location?.address?.city || 'N/A',
//     //   country: data.location?.address?.countryName || 'N/A',
//     //   starRating: data.starRating || null,
//     //   guestRating: data.guestReviews?.rating || null,
//     //   reviewCount: data.guestReviews?.total || 0,
//     //   amenities:
//     //     data.propertyContentSectionGroups?.amenities?.sections
//     //       ?.map(s => s.text || s.sectionName)
//     //       .filter(Boolean) || [],
//     //   policies:
//     //     data.propertyContentSectionGroups?.policies?.sections?.flatMap(section => {
//     //       const title = section.header?.text || section.sectionName || 'N/A';
//     //       return (section.bodySubSections || []).flatMap(sub =>
//     //         (sub.elements || []).map(el => el.header?.text || 'N/A')
//     //       ).map(text => ({ title, text }));
//     //     }) || [],
//     //   specialFeatures:
//     //     data.propertyContentSectionGroups?.specialFeatures?.sections
//     //       ?.map(s => s.text || s.sectionName)
//     //       .filter(Boolean) || [],
//     // };
//
...
//     );
//
//     const hotelsData = overpassRes.data.elements.map(hotel => hotel);
//     const savedHotels = [];
//     const usedImages = new Set();
//
//
//     console.log(hotelsData, 1111111111111)
//
//     // Room types & amenities
//     const allAmenities = [
//       "Indoor pool", "Free Wi-Fi", "AirPorts.js transfer", "Free parking",
//       "Spa & wellness center", "Family rooms", "Fitness center",
//       "Non-smoking rooms", "Bar", "Excellent breakfast", "Room service"
//     ];
//
//     const amenitiesByRoomType = {
//       "Family Suite": ["Spa & wellness center", "Free Wi-Fi", "Excellent breakfast", "Fitness center", "Free parking"],
//       "Apartment": ["Free Wi-Fi", "Excellent breakfast", "Free parking"],
//       "Standard Double Room with 1 Bed": ["Free Wi-Fi", "Non-smoking rooms"],
//       "Standard Double Room with 2 Single Beds": ["Free Wi-Fi", "Non-smoking rooms"],
//       "Deluxe Double Room with 1 Bed": ["Free Wi-Fi", "Excellent breakfast", "Fitness center"],
//       "Deluxe Double Room with 2 Single Beds": ["Free Wi-Fi", "Excellent breakfast", "Fitness center"],
//       "Presidential Suite": allAmenities
//     };
//
//     const roomTypes = [
//       { type: "Family Suite", beds: ["2 single beds","1 large double bed"], maxGuests: 4, price: 200 },
//       { type: "Apartment", beds: ["1 large double bed"], maxGuests: 2, price: 150 },
//       { type: "Standard Double Room with 1 Bed", beds: ["1 large double bed"], maxGuests: 2, price: 100 },
//       { type: "Standard Double Room with 2 Single Beds", beds: ["2 single beds"], maxGuests: 2, price: 100 },
//       { type: "Deluxe Double Room with 1 Bed", beds: ["1 large double bed"], maxGuests: 2, price: 130 },
//       { type: "Deluxe Double Room with 2 Single Beds", beds: ["2 single beds"], maxGuests: 2, price: 130 },
//       { type: "Presidential Suite", beds: ["Bedroom 1: extra large double bed","Bedroom 2: extra large double bed"], maxGuests: 4, price: 500 }
//     ];
//
//     // Utility: pick one unique image per category
//     function pickOneUniqueImage(category) {
//       const available = allImages[category].filter(img => !usedImages.has(img));
//       if (!available.length) throw new Error(`No more unique images left in category ${category}`);
//       const idx = Math.floor(Math.random() * available.length);
//       usedImages.add(available[idx]);
...
//     //     lat: h.lat ? parseFloat(h.lat) : null,
//     //     lon: h.lon ? parseFloat(h.lon) : null,
//     //     stars: tags.stars ? parseFloat(tags.stars.replace(',', '.')) : 4.0,
...
//     //     where: { name: hotelValues.name, cityName: city }
//     //   });
//     //
//     //   let hotelInstance;
//     //   let created = false;
//     //
//     //   if (existingHotel) {
//     //     hotelInstance = existingHotel;
//     //   } else {
//     //     hotelInstance = await Hotel.create(hotelValues);
//     //     created = true;
//     //   }
//     //
//     //   console.log(hotelInstance,333)
//     //   // savedHotels.push(hotelsData);
//     //   savedHotels.push(...hotelsData);
//     //
//     //   console.log(created,222222222)
//     //   // Ստեղծել room-ներ միայն նոր hotels-ի համար
//     //   if (created) {
...
//     //         amenities: amenitiesByRoomType[room.type] || [],
//     //         description: `Comfortable ${room.type} at our hotel.`
//     //       });
//     //
//     //       // Նկարներ
//     //       const roomImages = {
//     //         bedroom: pickOneUniqueImage("bedroom"),
//     //         bathroom: pickOneUniqueImage("bathroom"),
//     //         lobby: pickOneUniqueImage("lobby"),
//     //         view: pickOneUniqueImage("view")
//     //       };
//     //
//     //       for (const category of ["bedroom","bathroom","lobby","view"]) {
//     //         await Photo.create({
//     //           hotelId: hotelInstance.id,
//     //           roomId: roomInstance.id,
//     //           path: roomImages[category]
//     //         });
//     //       }
//     //     }
//     //   }
...
//     //
//     //
//     // res.json({ count: savedHotels.length, hotels: hotelsData });
//
//
//     for (let h of hotelsData) {
//       const tags = h.tags || {};
...
//
//       // ✅ ճիշտ push
//       savedHotels.push(hotelInstance);
//
//       if (created) {
//         for (const room of roomTypes) {
//           const roomInstance = await Room.create({
...
//       };
//
//       const [hotelInstance, created] = await Hotel.findOrCreate({
//         where: { name: hotelValues.name, cityName: city },
//         defaults: hotelValues,
//
//       });
//
//       savedHotels.push(hotelInstance);
//       console.log(savedHotels, 1111111111111)
//       // 3️⃣ Create Room (կենտրոնացված օրինակ)
//       // const room = await Room.create({
//       //   hotelId: hotel.id,
//       //   type: "Standard",
//       //   price: 0,
//       //   beds: 1,
//       //   amenities: hotelValues.amenities,
//       //   description: hotelValues.description
//       // });
//
//
//       // Полный список всех доступных удобств
//       const allAmenities = [
//         "Indoor pool",
//         "Free Wi-Fi",
//         "AirPorts.js transfer",
//         "Free parking",
//         "Spa & wellness center",
//         "Family rooms",
//         "Fitness center",
...
//       ];
//
// // Настройка удобств для каждого типа номера
//       const amenitiesByRoomType = {
//         "Family Suite": [
//           "Spa & wellness center",
//           "Free Wi-Fi",
...
//           "Non-smoking rooms"
//         ],
//         "Standard Double Room with 2 Single Beds": [
//           "Free Wi-Fi",
//           "Non-smoking rooms"
//         ],
...
//           "Fitness center"
//         ],
//         "Presidential Suite": allAmenities // All amenities
//       };
//
// // Типы номеров
//       const roomTypes = [
...
//         },
//         {
//           type: "Standard Double Room with 1 Bed",
//           beds: ["1 large double bed"],
//           maxGuests: 2,
//           price: 100
//         },
//         {
//           type: "Standard Double Room with 2 Single Beds",
//           beds: ["2 single beds"],
//           maxGuests: 2,
//           price: 100
//         },
//         {
//           type: "Deluxe Double Room with 1 Bed",
//           beds: ["1 large double bed"],
//           maxGuests: 2,
//           price: 130
//         },
//         {
//           type: "Deluxe Double Room with 2 Single Beds",
//           beds: ["2 single beds"],
//           maxGuests: 2,
//           price: 130
//         },
...
//             "Bedroom 2: extra large double bed"
//           ],
//           maxGuests: 4,
//           price: 500
//         }
//       ];
//
// // Создание комнат с заранее заданными удобствами
// //       for (const room of roomTypes) {
// //         await Room.create({
// //           hotelId: hotel.id,
// //           type: room.type,
...
// //
// //
// //       console.log("Все номера созданы с индивидуальными удобствами!");
//
//
//       // Assume `hotel` is an instance of Hotel already created
//
// // Array of rooms with English types and amenities
//       const rooms = roomTypes.map(room => ({
//         hotelId: hotelInstance.id,
//         type: room.type,
//         price: room.price,
//         beds: room.beds.length, // count of beds
//         maxGuests: room.maxGuests,
//         description: `Comfortable ${room.type} at our hotel.`,
//         amenities: amenitiesByRoomType[room.type] || []
//       }));
...
//       }
//
//       console.log("All rooms have been successfully created!");
//
//
//       function pickOneUniqueImage(category, usedImages = new Set()) {
//         const available = allImages[category].filter(img => !usedImages.has(img));
//         if (available.length === 0) {
//           throw new Error(`No more unique images left in category ${category}`);
//         }
//         const randomIndex = Math.floor(Math.random() * available.length);
//         const img = available[randomIndex];
//         usedImages.add(img);
//         return img;
//       }
//
// // 3️⃣ Seed կոնֆիգ
//       const totalRooms = 40;
//       const roomsPerHyuranoci = 20;
//       const totalHyuranoci = Math.ceil(totalRooms / roomsPerHyuranoci);
//
//       const usedImages = new Set();
//       const allRooms = [];
//
...
//         });
//         createdRooms.push(roomInstance);
//       }
//
// // 3️⃣ Create photos
//       for (const roomInstance of createdRooms) {
//         const roomData = {
//           bedroom: pickOneUniqueImage("bedroom", usedImages),
//           bathroom: pickOneUniqueImage("bathroom", usedImages),
//           lobby: pickOneUniqueImage("lobby", usedImages),
//           view: pickOneUniqueImage("view", usedImages)
//         };
//
//         for (const category of ["bedroom", "bathroom", "lobby", "view"]) {
//           await Photo.create({
...
//
//
// // 4️⃣ Ստեղծում ենք room-ներ ու Photo
// //       for (let h = 1; h <= totalHyuranoci; h++) {
// //         const hyuranociRooms = [];
// //         for (let r = 1; r <= roomsPerHyuranoci; r++) {
// //           const roomData = {
// //             bedroom: pickOneUniqueImage("bedroom", usedImages),
// //             bathroom: pickOneUniqueImage("bathroom", usedImages),
// //             lobby: pickOneUniqueImage("lobby", usedImages),
...
// //
// //
// //           // Ստեղծում ենք Photo record-ները
// //           for (const category of ["bedroom", "bathroom", "lobby", "view"]) {
// //             await Photo.create({
// //               hotelId: h, // կամ համապատասխան hotel.id
// //               roomId: room.id,
// //               path: roomData[category]
// //             });
// //           }
// //
// //           hyuranociRooms.push(room);
// //         }
// //         allRooms.push({hyuranociId: h, rooms: hyuranociRooms});
// //       }
//
//     }
//
...
//
//
//     res.json({count: savedHotels.length, hotels: savedHotels});
//
//   } catch (err) {
//     console.error(err.response?.status, err.response?.data || err.message);
//     res.status(500).json({error: 'Server error'});
